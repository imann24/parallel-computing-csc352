...
for line in sys.stdin:
    ...
    # if there's a double quote in the line, the format
    # is raw-life
    isRawLife = line.find( "\"" ) != -1

    # if raw-life, figure out where the live cells are.
    if isRawLife:
       lineNo = int( words[0] )
    ...
    # if not raw-life, the format of the input is 
    # r c 
    # indicating the row and column of a live cell
    else:
       lineNo = int( words[0] )
       j      = int( words[1] )
       print( "%03d%03d\t%s" % ( lineNo, j, "alive" ) )
